...
// 3. API для получения списка товаров;
// нам также надо будет знать, сколько всего товаров,
// поэтому вводим промежуточный объект с этой информацией
interface IListResult<T> { // объявляем дженерик, где T — любой тип
    total: number; // количество элементов в результате
    items: T[] // список элементов
}

// 4. API оформления заказа
// Интерфейс запроса
...
// Интерфейс отрицательного ответа сервера
...
interface ILarekAPI {
    ...
    loadProductList(): Promise<IListResult<IProductItem>>; // возвращаем список товаров в виде IListResult асинхронно
    // оформить заказ
    orderProducts: (order: IOrderRequest) => Promise<IOrderResponse>; // отправляем заказ на сервер и получаем id созданого заказа асинхронно
    // получить конкретный товар
    getProduct: (id: string) => Promise<IProductItem>;
}

// 4. Товары нужно где-то хранить, но список может быть и универсальным;
// также нужно сохранить состояние страницы — актуальный запрос, счетчик корзины
// и обрисовать набор возможных действий на странице
...
### Шаг 2: Реализуем модели данных
// 1. Товар
class Product implements IProductItem {
    id: string;
    description: string;
    image: string;
    title: string;
...
// 3. Список
// Тип для передачи класса элементов списка.
// Здесь D — это тип данных на входе,
// T — это тип, который получим после инстанцирования, он расширяет D
interface IItemConstructor<D, T extends D> {
    new (data: D): T; // типовой конструктор модели, получаем данные и мимикрируем под них
}

// Не будем привязываться к конкретному API, но декларируем, что нам нужно
// следуем принципам "инверсии зависимостей" и "разделения интерфейсов"
interface IListAPI<T> {
    loadProductList(): Promise<IListResult<T>>;
}

interface IProductAPI extends IListAPI<IProductItem> {}

// Список сразу делаем универсальным — неважно для постов он или чего-то еще
// Список будет получать данные в формате DataType и приводить к ItemType
class ItemsList<ItemType extends DataType, DataType> implements IListModel<ItemType> {
    protected _items: ItemType[] = []; // элементы списка

    // передаем класс элемента и API для получения данных
    constructor(
        protected Item: IItemConstructor<ItemType>,
        protected api: IListAPI<DataType>
    ) {}

    // прячем _items в сеттер, чтобы обеспечивать конвертацию данных
    set items(data: ItemType[]) {
        this._items = data.map(item => new this.Item(item));
    }

    // возвращаем преобразованные к нужному классу данные
    get items(): ItemType[] {
        return this._items;
    }

    // базовый метод для загрузки элементов списка
    async load() {
        const result = await this.api.loadProductList();
    }
}